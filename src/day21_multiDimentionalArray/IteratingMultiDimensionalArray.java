...
public class IteratingMultiDimensionalArray {
   ...
   //index of 1D array   0               1               2
        ...
        System.out.println("*************************************");
        for (int i = arr2D.length - 1; i >= 0; i--) {//each single array
            for (int j = arr2D[i].length - 1; j >= 0; j--) {//each element
        ...
        for (int[] each1DArray : arr2D) {//for each loop start always from beginning
           for (int eachElement : each1DArray){
               System.out.println(eachElement);
           }
